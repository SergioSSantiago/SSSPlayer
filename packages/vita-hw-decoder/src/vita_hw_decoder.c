...
#define VITA_HW_DECODER_OPEN_DEADLINE_US (5ULL * 1000ULL * 1000ULL)
#define VITA_HW_DECODER_AUDIO_READY_DEADLINE_US (1500ULL * 1000ULL)
...
#define VITA_HW_DECODER_IO_STOP_GRACE_US (2ULL * 1000ULL * 1000ULL)
#define VITA_HW_DECODER_AUDIO_START_THREAD_PRIORITY 0x10000100
#define VITA_HW_DECODER_AUDIO_START_THREAD_STACK 0x40000
#define VITA_HW_DECODER_SEEK_OVERSHOOT_US (2ULL * 1000ULL * 1000ULL)
#define VITA_HW_DECODER_AV_LANDING_TOLERANCE_US (500ULL * 1000ULL)
...
#define VITA_HW_DECODER_SEEK_FIRST_FRAME_US (8ULL * 1000ULL * 1000ULL)
...
static int input_interrupted(const VitaHwDecoderInput *input) {
...
static void input_io_lock(VitaHwDecoderInput *input) {
	while (__sync_lock_test_and_set(&input->io_lock, 1))
		sceKernelDelayThread(100);
}

static void input_io_unlock(VitaHwDecoderInput *input) {
	__sync_lock_release(&input->io_lock);
}

static int input_begin_io(VitaHwDecoderInput *input) {
	if (!input) return 0;
	input_io_lock(input);
...
static void input_end_io(VitaHwDecoderInput *input) {
	if (!input) return;
	input_io_lock(input);
	input->io_active = 0;
	__sync_synchronize();
...
static int input_abort_active_io(VitaHwDecoderInput *input) {
	if (!input) return 0;
	input_io_lock(input);
...
static void input_publish_cancel(VitaHwDecoderInput *input) {
	if (!input) return;
	input_io_lock(input);
...
static int input_cancel_and_abort(VitaHwDecoderInput *input) {
	input_publish_cancel(input);
	return input_abort_active_io(input);
}

static int input_abort_after_grace(VitaHwDecoderInput *input,
	                               int worker_started,
...
static void input_clear_cancel_if_reusable(VitaHwDecoderInput *input) {
	if (!input) return;
	input_io_lock(input);
...
struct VitaHwDecoderPlayer {
	...
	VitaHwDecoderTrackInfo audio_tracks[VITA_HW_DECODER_MAX_AUDIO_TRACKS];
	VitaHwDecoderTrackInfo subtitle_tracks[VITA_HW_DECODER_MAX_SUBTITLE_TRACKS];
...
static void audio_operation_lock(VitaHwDecoderPlayer *player) {
	while (__sync_lock_test_and_set(&player->audio_operation_lock, 1))
		sceKernelDelayThread(100);
}

static void audio_operation_unlock(VitaHwDecoderPlayer *player) {
	__sync_lock_release(&player->audio_operation_lock);
}

static int audio_operation_begin(VitaHwDecoderPlayer *player) {
...
static void audio_operation_end(VitaHwDecoderPlayer *player,
	                            int commit_audio) {
	audio_operation_lock(player);
	input_clear_cancel_if_reusable(&player->audio_input);
	player->audio_operation_interrupted = 0;
...
static int audio_operation_cancelled(const VitaHwDecoderPlayer *player,
...
static int audio_operation_commit(VitaHwDecoderPlayer *player,
	                              volatile int *operation_cancel) {
	audio_operation_lock(player);
...
static void audio_operation_begin_rollback(VitaHwDecoderPlayer *player) {
	audio_operation_lock(player);
...
static void fill_track_info(VitaHwDecoderTrackInfo *out,
...
static void clear_track_snapshot(VitaHwDecoderPlayer *player) {
	if (!player) return;
...
static const VitaHwDecoderStreamFactory *
player_audio_factory(const VitaHwDecoderPlayer *player)
{
...
static int player_has_separate_audio(const VitaHwDecoderPlayer *player)
{
	return player &&
...
static void snapshot_tracks(VitaHwDecoderPlayer *player,
	...
	for (unsigned int i = 0; format && i < format->nb_streams; i++) {
		...
		if (playable_aac_stream(stream) &&
		    player->audio_track_count < VITA_HW_DECODER_MAX_AUDIO_TRACKS) {
...
static void stream_handle_close(VitaHwDecoderStreamHandle *stream) {
	if (!stream) return;
...
static void input_close(VitaHwDecoderInput *input) {
	if (!input) return;
	VitaHwDecoderStreamHandle detached = {0};
...
static int input_open(VitaHwDecoderInput *input,
	...
	input_io_unlock(input);
	if (input_interrupted(input)) return AVERROR_EXIT;
	VitaHwDecoderStreamHandle opened = {0};
	...
	input->deadline_us = sceKernelGetProcessTimeWide() +
	                     VITA_HW_DECODER_OPEN_DEADLINE_US;
...
static int ensure_audio_input_reusable(VitaHwDecoderPlayer *player,
	                                   volatile int *operation_cancel) {
...
static int audio_start_thread(SceSize args, void *argp) {
	(void)args;
	VitaHwDecoderAudioStartJob *job =
	    *(VitaHwDecoderAudioStartJob **)argp;
	job->result = vita_hw_elementary_audio_start(
...
static int start_audio_candidate(VitaHwDecoderPlayer *player,
	...
	SceUID thid = sceKernelCreateThread(
	    "VitaHwDecoderAudioStart", audio_start_thread,
	    VITA_HW_DECODER_AUDIO_START_THREAD_PRIORITY,
	    VITA_HW_DECODER_AUDIO_START_THREAD_STACK, 0, 0, NULL);
	if (thid < 0) return thid;
	VitaHwDecoderAudioStartJob *job_ptr = &job;
...
static void close_session(VitaHwDecoderPlayer *player) {
	...
	if (player->video.started || player->video.buffers_retained) {
		if (player->video_gpu_fenced)
			vita_hw_software_video_join_after_gpu_fence(&player->video);
		else
			vita_hw_software_video_join(&player->video);
	}
...
static int seek_input_ms(VitaHwDecoderInput *input, int stream_index,
	                     uint64_t position_ms) {
...
static void stop_audio_candidate(VitaHwDecoderPlayer *player) {
	...
	uint64_t stop_deadline = sceKernelGetProcessTimeWide() +
	                         VITA_HW_DECODER_IO_STOP_GRACE_US;
...
static int restart_audio_track(VitaHwDecoderPlayer *player, int audio_track,
	                           uint64_t position_ms,
	                           volatile int *operation_cancel,
	...
	uint64_t readiness_deadline = sceKernelGetProcessTimeWide() +
	                              VITA_HW_DECODER_AUDIO_READY_DEADLINE_US;
	...
	if (require_initial_packet && !player->audio.first_packet_ready) {
		...
		log_printf("decoder audio switch: no AU at %llu ms ret=%d track=%d deadline=%u ms",
		           ...
		           (unsigned)(VITA_HW_DECODER_AUDIO_READY_DEADLINE_US / 1000ULL));
		stop_audio_candidate(player);
	...
	if (require_initial_packet && player->audio.first_packet_pts_valid &&
	    (uint64_t)player->audio.first_packet_pts_ms * 1000ULL >
	        position_ms * 1000ULL + VITA_HW_DECODER_SEEK_OVERSHOOT_US) {
	...
	vita_hw_elementary_audio_set_volume(&player->audio,
...
static void abandon_audio_replacement(VitaHwDecoderPlayer *player,
	                                  uint64_t position_ms) {
	stop_audio_candidate(player);
	player->has_audio = 0;
	player->audio_clock_handed_off = 0;
...
static int restart_session_in_place(VitaHwDecoderPlayer *player,
	                                uint64_t position_ms) {
	uint64_t started_us = sceKernelGetProcessTimeWide();
	...
	if (player->video.started || player->video.buffers_retained) {
		if (player->video_gpu_fenced)
			vita_hw_software_video_join_for_restart(&player->video);
		else
			vita_hw_software_video_join(&player->video);
	}
	if (player->audio.started || player->audio.decoder_ready || player->audio.port_open)
	...
	if (player->has_audio) {
		uint64_t audio_start_deadline = sceKernelGetProcessTimeWide() +
		                                VITA_HW_DECODER_SEEK_FIRST_FRAME_US;
	...
	uint64_t deadline = restarted_us + VITA_HW_DECODER_SEEK_FIRST_FRAME_US;
	...
	while (!*player->cancel && !player->video.had_error && !player->video.eof &&
		...
		vita_hw_software_video_buffer_status(&player->video, &ready, NULL,
		                                     &first_ready_pts_us);
	...
	if (first_ready_pts_us < requested_us) {
		...
		player->clock_us = first_ready_pts_us;
	                               ...
	                               VITA_HW_DECODER_SEEK_OVERSHOOT_US) {
	...
	if (player->has_audio && player->audio.first_packet_pts_valid) {
		...
		if (landing_delta_us > VITA_HW_DECODER_AV_LANDING_TOLERANCE_US) {
...
static int open_session(VitaHwDecoderPlayer *player, uint64_t start_position_ms) {
	...
	if (player_has_separate_audio(player)) {
		...
		/* Keep text subtitles from the video container when present. */
		{
	...
	/* Video is allowed to decode while audio waits on the start gate. Do not publish
	 * an opened session until the decoder has produced an actual picture: an
	 * accepted-but-silent h264_vita session otherwise plays audio forever behind
	 * the Preparing video overlay. AUTO can immediately reopen the CPU backend
	 * when this bounded hardware attempt fails. */
	uint64_t gate_started_us = sceKernelGetProcessTimeWide();
	uint64_t deadline = gate_started_us + 2500000ULL;
	int ready = 0;
	while (!*player->cancel && !player->video.had_error && !player->video.eof &&
	       !(player->has_audio && player->audio.had_error) &&
	       sceKernelGetProcessTimeWide() < deadline) {
	...
	if (player->has_audio && player->audio.first_packet_pts_valid) {
		...
		if (audio_pts_us > requested_us +
		                   VITA_HW_DECODER_SEEK_OVERSHOOT_US ||
		    landing_delta_us > VITA_HW_DECODER_AV_LANDING_TOLERANCE_US) {
...
int vita_hw_decoder_select_audio_track_with_cancel(
	...
	if (selected_end_ms && position_ms >= selected_end_ms) {
		...
		return VITA_HW_DECODER_AUDIO_CHANGE_ROLLED_BACK;
	}
	...
	if (restore_ret >= 0) {
		if (audio_operation_commit(
			...
			return ret == AVERROR_EXIT
			     ? AVERROR_EXIT : VITA_HW_DECODER_AUDIO_CHANGE_ROLLED_BACK;
		}
...
int vita_hw_decoder_select_audio_track(VitaHwDecoderPlayer *player,
	                                   int audio_track,
	                                   uint64_t position_ms) {
	return vita_hw_decoder_select_audio_track_with_cancel(
...
static int audio_is_clock_master(const VitaHwDecoderPlayer *player) {
...
static uint64_t presentation_clock(VitaHwDecoderPlayer *player) {
	...
	if (!audio_is_clock_master(player)) {
		...
		uint64_t raw = player->has_audio
		             ? vita_hw_elementary_audio_clock_us(&player->audio) : 0;
...
int vita_hw_decoder_present(VitaHwDecoderPlayer *player, int fill_screen) {
	...
	uint64_t raw = audio_is_clock_master(player)
	             ? vita_hw_elementary_audio_clock_us(&player->audio) : clock;
	return vita_hw_software_video_present(&player->video, clock, raw,
...
void vita_hw_decoder_discard_video_to_clock(VitaHwDecoderPlayer *player) {
	...
	uint64_t clock = presentation_clock(player);
	vita_hw_software_video_discard_to_clock(&player->video, clock);
}
...
void vita_hw_decoder_get_status(VitaHwDecoderPlayer *player,
	...
	if (audio_us > 0 && player->clock_us > audio_us +
	    VITA_HW_DECODER_CLOCK_WINDOW_US * 4ULL) {
	...
	/* The presenter retains the most recently displayed surface between video
	 * deadlines. Count it as available so a 24 fps stream rendered by a 60 Hz UI
	 * does not report false buffering for the intervening frames. */
