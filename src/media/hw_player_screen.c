...
#define SEEK_REPEAT_DELAY_US 280000ULL
#define SEEK_REPEAT_INTERVAL_US 160000ULL
#define SEEK_STEP_FINE_MS 1000LL
#define SEEK_STEP_MED_MS 5000LL
#define SEEK_STEP_COARSE_MS 10000LL
#define SEEK_ACCEL_MED_US 800000ULL
#define SEEK_ACCEL_COARSE_US 1800000ULL
#define TIMELINE_HIT_PAD 28
...
static int run_seek(VtDecoderPlayer *player, UiPlayerLoadingInfo *loading,
	...
	/* Seek must not inherit open()'s abort cancel_action: a timeline scrub hold
	 * (≥800 ms) was being re-read as "cancel hold" on the loading scene, which
	 * aborted the transport and skipped recovery — HUD moved, picture did not. */
	UiPlayerLoadingInfo seek_loading;
	memset(&seek_loading, 0, sizeof(seek_loading));
	if (loading) seek_loading = *loading;
	seek_loading.cancel_action = NULL;
	seek_loading.cancel_ctx = NULL;
	seek_loading.start_over_requested = NULL;
	ui_touch_reset();
	return ui_player_loading_run(&seek_loading, seek_task, &task, cancel,
	                             NULL, NULL);
}
...
static uint64_t resync_after_blocking_action(SceCtrlData *controls,
	...
	if (stick_hold_started) *stick_hold_started = 0;
	ui_touch_reset();
...
/* Snap scrub/seek targets to whole seconds so the HUD and landing agree. */
static uint64_t snap_seek_ms(uint64_t position_ms, uint64_t duration_ms) {
	uint64_t snapped = (position_ms / 1000ULL) * 1000ULL;
	if (duration_ms && snapped > duration_ms) snapped = duration_ms;
	return snapped;
}

static int64_t seek_step_ms_for_hold(uint64_t held_us) {
	if (held_us >= SEEK_ACCEL_COARSE_US) return SEEK_STEP_COARSE_MS;
	if (held_us >= SEEK_ACCEL_MED_US) return SEEK_STEP_MED_MS;
	return SEEK_STEP_FINE_MS;
}
...
static void mark_hud_seek(int *hold, uint64_t *hold_ms, uint64_t *hold_until,
	                      uint64_t now_us, uint64_t target_ms) {
	if (!hold || !hold_ms || !hold_until) return;
	*hold = 1;
	*hold_ms = target_ms;
	*hold_until = now_us + 2500000ULL;
}

static float timeline_fraction_from_touch(int touch_x) {
	float fraction = (float)(touch_x - TIMELINE_X) / (float)TIMELINE_W;
...
static void draw_hud(const VtHwPlayerScreenSource *source,
	                 ...
	                 int scrubbing, uint64_t scrub_ms,
	...
	if (hud_opacity > 0.01f) {
		...
		float progress;
		if (dragging) progress = drag_fraction;
		else if (scrubbing && status->duration_ms)
			progress = (float)((double)scrub_ms / (double)status->duration_ms);
		else if (status->duration_ms)
			progress = (float)((double)status->position_ms /
			                   (double)status->duration_ms);
		else progress = 0.0f;
		if (progress < 0.0f) progress = 0.0f;
		if (progress > 1.0f) progress = 1.0f;
		int show_scrub = dragging || scrubbing;
		float timeline_h = show_scrub ? 10.0f : 6.0f;
		vita2d_draw_rectangle(TIMELINE_X, TIMELINE_Y, TIMELINE_W, timeline_h,
		...
		if (has_buffer &&
			...
			if (buffer_end > buffer_start) {
				...
				vita2d_draw_rectangle(TIMELINE_X + TIMELINE_W * buffer_start,
				                      TIMELINE_Y, buffer_width, timeline_h,
				                      fade_color(VT_THEME_SIGNAL_LIGHT, hud_opacity));
			}
		}
		vita2d_draw_rectangle(TIMELINE_X, TIMELINE_Y, TIMELINE_W * progress,
		                      timeline_h,
		                      fade_color(VT_THEME_BLUE_BRIGHT, hud_opacity));
		vita2d_draw_fill_circle(TIMELINE_X + TIMELINE_W * progress,
		                        TIMELINE_Y + timeline_h * 0.5f,
		                        show_scrub ? 11.0f : 6.0f, text);
		char current[24], duration[24];
		uint64_t shown_position = status->position_ms;
		uint64_t shown_duration = status->duration_ms;
		if (dragging && shown_duration)
			shown_position = snap_seek_ms(
			    (uint64_t)(drag_fraction * (double)shown_duration),
			    shown_duration);
		else if (scrubbing) shown_position = scrub_ms;
		format_time(shown_position, current);
		format_time(shown_duration, duration);
		if (small) {
...
int vt_hw_player_screen_run(const VtHwPlayerScreenSource *source,
	...
	uint64_t drag_target_ms = 0;
	uint64_t timeline_duration_ms = 0;
	int hud_seek_hold = 0;
	uint64_t hud_seek_ms = 0;
	uint64_t hud_seek_hold_until = 0;
	...
	uint64_t left_seek_hold_started = 0;
	int drag_resume_playing = 0;
	...
	for (;;) {
		...
		if (lock_changed) {
			if (dragging && drag_resume_playing) {
				paused = 0;
				vt_decoder_set_paused(player, 0);
			}
			dragging = 0;
			drag_resume_playing = 0;
			sidebar.open = 0;
			right_open = 0;
			hud_visible = 1;
		...
		if (status.duration_ms) timeline_duration_ms = status.duration_ms;
		if (status.error) {
			if (decoder_preference == VT_VIDEO_DECODER_AUTO &&
				...
				if (ret >= 0) {
					...
					now = resync_after_blocking_action(
					    ...
					    &left_seek_repeat_at, &left_seek_hold_started);
					hud_deadline = now + 3500000ULL;
					continue;
				}
			}
		...
		unsigned held = controls.buttons;
		int digital_hold_seek = 0;
		if (!panels_open) {
			if (swapped) {
				if (held & SCE_CTRL_RTRIGGER) digital_hold_seek = 1;
				else if (held & SCE_CTRL_LTRIGGER) digital_hold_seek = -1;
			} else {
				if (held & SCE_CTRL_RIGHT) digital_hold_seek = 1;
				else if (held & SCE_CTRL_LEFT) digital_hold_seek = -1;
			}
		}
		int edge_seek_direction = !panels_open &&
		...
		int analog_seek_direction = !panels_open && abs_left_x > STICK_DEADZONE &&
		                          ...
		                          ? (left_x < 0 ? -1 : 1) : 0;
		int hold_seek_direction = digital_hold_seek ? digital_hold_seek
		                        : analog_seek_direction;
		int seek_direction = 0;
		if (!hold_seek_direction) {
			left_seek_direction = 0;
			left_seek_repeat_at = 0;
			left_seek_hold_started = 0;
			if (edge_seek_direction) seek_direction = edge_seek_direction;
		} else if (hold_seek_direction != left_seek_direction) {
			left_seek_direction = hold_seek_direction;
			left_seek_hold_started = now;
			left_seek_repeat_at = now + SEEK_REPEAT_DELAY_US;
			seek_direction = hold_seek_direction;
		} else if (now >= left_seek_repeat_at) {
			left_seek_repeat_at = now + SEEK_REPEAT_INTERVAL_US;
			seek_direction = hold_seek_direction;
		}
		/* Immediate seeks (original reliable path). Tap = 1s; hold accelerates. */
		if (seek_direction) {
			uint64_t duration = status.duration_ms;
			uint64_t held_us = left_seek_hold_started
			                 ? now - left_seek_hold_started : 0;
			int64_t step_ms = seek_step_ms_for_hold(held_us);
			uint64_t base = snap_seek_ms(status.position_ms, duration);
			int64_t target = (int64_t)base + seek_direction * step_ms;
			if (target < 0) target = 0;
			if (duration && (uint64_t)target > duration)
				target = (int64_t)duration;
			target = (int64_t)snap_seek_ms((uint64_t)target, duration);
			if ((uint64_t)target != base) {
				int preserved_dir = hold_seek_direction;
				uint64_t preserved_hold = left_seek_hold_started
				                       ? left_seek_hold_started : now;
				int seek_ret = run_seek(player, &loading, &cancel,
				                        (uint64_t)target);
				now = resync_after_blocking_action(
				    &controls, &previous, &left_seek_direction,
				    &left_seek_repeat_at, &left_seek_hold_started);
				if (preserved_dir) {
					unsigned held_after = controls.buttons;
					int still = 0;
					if (swapped) {
						still = (preserved_dir > 0 &&
						         (held_after & SCE_CTRL_RTRIGGER)) ||
						        (preserved_dir < 0 &&
						         (held_after & SCE_CTRL_LTRIGGER));
					} else {
						still = (preserved_dir > 0 &&
						         (held_after & SCE_CTRL_RIGHT)) ||
						        (preserved_dir < 0 &&
						         (held_after & SCE_CTRL_LEFT));
					}
					int ax = (int)controls.lx - STICK_CENTER;
					int ay = (int)controls.ly - STICK_CENTER;
					int absx = ax < 0 ? -ax : ax;
					int absy = ay < 0 ? -ay : ay;
					if (!still && absx > STICK_DEADZONE && absx > absy &&
					    ((ax < 0) ? -1 : 1) == preserved_dir)
						still = 1;
					if (still) {
						left_seek_direction = preserved_dir;
						left_seek_hold_started = preserved_hold;
						left_seek_repeat_at = now + SEEK_REPEAT_INTERVAL_US;
					}
				}
				(void)seek_ret;
				if (seek_ret >= 0)
					mark_hud_seek(&hud_seek_hold, &hud_seek_ms,
					              &hud_seek_hold_until, now, (uint64_t)target);
				hud_visible = 1;
				hud_deadline = now + 3500000ULL;
				continue;
			}
		...
		int hud_interactive = hud_visible || hud_opacity > 0.65f || dragging;
		/* Full-width bottom strip: the 6px timeline alone is too easy to miss. */
		if (!panels_open &&
		    (touch_flags & UI_TOUCH_EVENT_DOWN) &&
		    touch.y >= 400 && touch.y < SCREEN_HEIGHT) {
			dragging = 1;
			drag_resume_playing = !paused;
			/* Do not pause the decoder while scrubbing. Pausing made touch seeks
			 * take a different path than D-pad (and a failed/skipped seek left
			 * playback stuck paused with the thumb snapped to 0). */
			hud_visible = 1;
			hud_deadline = now + 3500000ULL;
			drag_fraction = timeline_fraction_from_touch(touch.x);
			if (timeline_duration_ms) {
				drag_target_ms = snap_seek_ms(
				    (uint64_t)(drag_fraction * (double)timeline_duration_ms),
		...
		if (!panels_open && dragging &&
		    (touch_flags & (UI_TOUCH_EVENT_MOVE | UI_TOUCH_EVENT_HOLD |
		                    UI_TOUCH_EVENT_UP))) {
			/* Prefer the last tracked scrub position on UP: some lifts report a
			 * stale/zero sample that would otherwise seek back to 0:00. */
			if (!(touch_flags & UI_TOUCH_EVENT_UP) || touch.x >= TIMELINE_X) {
				drag_fraction = timeline_fraction_from_touch(touch.x);
				if (timeline_duration_ms) {
					drag_target_ms = snap_seek_ms(
					    (uint64_t)(drag_fraction * (double)timeline_duration_ms),
			...
			if (touch_flags & UI_TOUCH_EVENT_UP) {
				uint64_t duration = timeline_duration_ms
				                  ? timeline_duration_ms : status.duration_ms;
				uint64_t target = drag_target_ms;
				if (duration) {
					if (!target && drag_fraction > 0.0f) {
						target = snap_seek_ms(
						    (uint64_t)(drag_fraction * (double)duration),
						    duration);
					}
					drag_fraction = (float)((double)target / (double)duration);
				}
				int seek_ret = 0;
				int did_seek = 0;
				if (duration && target != status.position_ms) {
					seek_ret = run_seek(player, &loading, &cancel, target);
					did_seek = 1;
				}
				dragging = 0;
				now = resync_after_blocking_action(
					    &controls, &previous, &left_seek_direction,
					    &left_seek_repeat_at, &left_seek_hold_started);
				if (did_seek && seek_ret >= 0) {
					mark_hud_seek(&hud_seek_hold, &hud_seek_ms,
					              &hud_seek_hold_until, now, target);
				} else if (did_seek && seek_ret < 0) {
					hud_seek_hold = 0;
				}
				/* Always restore play/pause to what the user had before scrub. */
				if (drag_resume_playing) {
					paused = 0;
					vt_decoder_set_paused(player, 0);
				} else {
					paused = 1;
					vt_decoder_set_paused(player, 1);
				}
				drag_resume_playing = 0;
				hud_visible = 1;
				hud_deadline = now + 3500000ULL;
			...
			if (!hud_interactive) {
				...
				uint64_t target = status.position_ms > (uint64_t)SEEK_STEP_COARSE_MS
				                ? status.position_ms - (uint64_t)SEEK_STEP_COARSE_MS
				                : 0;
				target = snap_seek_ms(target, status.duration_ms);
				int seek_ret = run_seek(player, &loading, &cancel, target);
				now = resync_after_blocking_action(
					    &controls, &previous, &left_seek_direction,
					    &left_seek_repeat_at, &left_seek_hold_started);
				hud_visible = 1;
		...
		if (!paused && !dragging && now > hud_deadline)
			hud_visible = 0;
		...
		if (energy_saving) {
			...
			if (hud_seek_hold) {
				int64_t delta = (int64_t)status.position_ms - (int64_t)hud_seek_ms;
				if (delta < 0) delta = -delta;
				if (delta <= 1500 || now >= hud_seek_hold_until)
					hud_seek_hold = 0;
			}
			if (hud_opacity > 0.01f || volume_opacity > 0.01f)
				draw_hud(source, &status, paused, vt_audio_volume_percent(),
				         hud_opacity, volume_opacity,
				         hud_seek_hold && !dragging, hud_seek_ms,
				         dragging, drag_fraction);
