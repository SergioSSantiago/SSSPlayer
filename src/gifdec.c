...
gd_open_gif(const char *fname)
{
    ...
    (int)fread(&bgidx, 1, 1, fd);
    /* Aspect Ratio */
    (int)fread(&aspect, 1, 1, fd);
    ...
    /* Read GCT */
    gif->gct.size = gct_sz;
    (int)fread(gif->gct.colors, 1, 3 * gif->gct.size, fd);
    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
...
discard_sub_blocks(gd_GIF *gif)
{
    ...
    do {
        (int)fread(&size, 1, 1, gif->fd);
        fseek(gif->fd, (long)(size), SEEK_CUR);
    } while (size);
}
...
read_plain_text_ext(gd_GIF *gif)
{
    if (gif->plain_text) {
        ...
        uint8_t cw, ch, fg, bg;
        long sub_block;
        fseek(gif->fd, (long)(1), SEEK_CUR); /* block size = 12 */
        tx = read_num(gif->fd);
        ty = read_num(gif->fd);
        tw = read_num(gif->fd);
        th = read_num(gif->fd);
        (int)fread(&cw, 1, 1, gif->fd);
        (int)fread(&ch, 1, 1, gif->fd);
        (int)fread(&fg, 1, 1, gif->fd);
        (int)fread(&bg, 1, 1, gif->fd);
        sub_block = ftell(gif->fd);
        gif->plain_text(gif, tx, ty, tw, th, cw, ch, fg, bg);
        fseek(gif->fd, sub_block, SEEK_SET);
    } else {
        /* Discard plain text metadata. */
        fseek(gif->fd, (long)(13), SEEK_CUR);
    }
...
read_graphic_control_ext(gd_GIF *gif)
{
    ...
    fseek(gif->fd, (long)(1), SEEK_CUR);
    (int)fread(&rdit, 1, 1, gif->fd);
    gif->gce.disposal = (rdit >> 2) & 3;
    gif->gce.input = rdit & 2;
    gif->gce.transparency = rdit & 1;
    gif->gce.delay = read_num(gif->fd);
    (int)fread(&gif->gce.tindex, 1, 1, gif->fd);
...
read_application_ext(gd_GIF *gif)
{
    ...
    (int)fread(app_id, 1, 8, gif->fd);
    /* Application Authentication Code. */
    (int)fread(app_auth_code, 1, 3, gif->fd);
    if (!strncmp(app_id, "NETSCAPE", sizeof(app_id))) {
        /* Discard block size (0x03) and constant byte (0x01). */
        fseek(gif->fd, (long)(2), SEEK_CUR);
...
get_key(gd_GIF *gif, int key_size, uint8_t *sub_len, uint8_t *shift, uint8_t *byte)
{
    ...
    for (bits_read = 0; bits_read < key_size; bits_read += frag_size) {
        ...
        if (rpad == 0) {
            /* Update byte. */
            if (*sub_len == 0) {
                (int)fread(sub_len, 1, 1, gif->fd); /* Must be nonzero! */
                if (*sub_len == 0)
                    return 0x1000;
            }
            (int)fread(byte, 1, 1, gif->fd);
            (*sub_len)--;
        }
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    free(table);
    if (key == stop)
        (int)fread(&sub_len, 1, 1, gif->fd); /* Must be zero! */
...
read_image(gd_GIF *gif)
{
    ...
    if (fisrz & 0x80) {
        ...
        (int)fread(gif->lct.colors, 1, 3 * gif->lct.size, gif->fd);
        gif->palette = &gif->lct;
    } else
        gif->palette = &gif->gct;
    /* Image Data. */
