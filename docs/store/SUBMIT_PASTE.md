# VitaDB / VitaAlive — paste-ready submission

VitaDB requires **your** logged-in account
(https://www.rinnegatamante.eu/vitadb/#/login). There is no public submit API.

Open https://www.rinnegatamante.eu/vitadb/#/submit while logged in and paste below.

NeoVitaDB staging PR (auto-tracks latest GitHub VPK):
https://github.com/robin994/NeoVitaDB-Catalog-Test/pull/11

---

## Form fields

**Name**
```
SSSPlayer
```

**Author**
```
SergioSSantiago
```

**Title ID**
```
SSSP00001
```

**Version**
```
1.1.34
```

**Type / Category** (pick closest to Utility / Media / Application)

**Source code**
```
https://github.com/SergioSSantiago/SSSPlayer
```

**Download URL**
```
https://github.com/SergioSSantiago/SSSPlayer/releases/download/v1.1.34/SSSPlayer-1.1.34.vpk
```

**Icon file**
```
assets/icons/icon0.png
```
(or upload from the VPK `sce_sys/icon0.png`)

**Short description**
```
Music and video player for PS Vita by SergioSSantiago. VitaWave-style music (EQ, visualizer, playlists, themes), VitaMediaDeck-class H.264 video with precise seek, network streaming (SMB, SFTP, WebDAV, Jellyfin), plus YouTube search/play/download and in-app update check.
```

**Long description**
```
SSSPlayer is an all-in-one media player for PlayStation Vita by SergioSSantiago.

Music:
- Browse ux0:/music and uma0:/music (MP3, FLAC, OGG, M4A/AAC)
- Now Playing with touch scrub on the progress bar
- 10-band equalizer, spectrum visualizer, M3U playlists
- Theme system (Terminus CRT theme by default)

Video:
- Browse ux0:/video, uma0:/video, ux0:/movies, uma0:/movies
- Hardware H.264 playback with exact seek / scrub
- Subtitles and audio tracks
- Network sources: SMB, SFTP, WebDAV, Jellyfin

YouTube:
- Search and stream playback on-device
- Download video (H.264 ~360p progressive MP4) or audio (M4A)
- Destination folder picker; in-app delete (Square); download progress

Extras:
- In-app check for updates (GitHub Releases)
- Local file browser with delete

Install:
1. Download SSSPlayer-x.y.z.vpk from GitHub Releases
2. Enable FTP in VitaShell
3. Copy the VPK to ux0: with Cyberduck
4. Install from VitaShell

Credits: built by SergioSSantiago. Thanks to VitaWave and VitaMediaDeck — SSSPlayer reuses foundations from both projects.

TITLEID: SSSP00001
Data: ux0:data/SSSPlayer/
License: GPL-3.0-only
Version: 1.1.34
```

**Screenshots** (upload on-device captures if the form asks)
1. Browser with music + video folders  
2. Now Playing  
3. Video playback  
4. YouTube search / download  
5. Settings  

---

## VitaAlive / PS Vita Alive

Catalog: https://github.com/VegettoSan/PSVitaAlive (`apps/sssplayer.json`).  
Bump version via PR when shipping a new release. Keep TITLEID `SSSP00001` identical.
